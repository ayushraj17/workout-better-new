...
# generate-artifacts
Source: https://docs.uniwind.dev/api/generate-artifacts
...
# getCSSVariable
Source: https://docs.uniwind.dev/api/get-css-variable
...
# Layout Direction
Source: https://docs.uniwind.dev/api/layout-direction
...
## Direction Variants (`rtl:` / `ltr:`)

Style based on the layout direction:

```tsx theme={null}
<Text className="ltr:text-left rtl:text-right">
  Aligned with the reading direction
</Text>

<View className="ltr:pl-4 rtl:pr-4" />
```

By default, the variants follow the global RTL state. To force a direction for a specific subtree, wrap it in [`LayoutDirection`](/api/layout-direction) (available in Uniwind 1.9.0+, Pro 1.4.0+):

```tsx theme={null}
import { LayoutDirection } from 'uniwind'

<LayoutDirection rtl>
  <Text className="ltr:text-left rtl:text-right">Forced RTL</Text>
</LayoutDirection>
```
...
<AccordionGroup>
  ...
  <Accordion title="How does style specificity work in Uniwind?" icon="layer-group">
    ...
    Inline styles always have higher priority than className styles, even when the class uses Tailwind's important modifier (`!`).
    ...
    **Result:** The background will be blue.
    ...
    Read the full [Style Specificity](/style-specificity) guide for className conflicts, important utilities like `!bg-red-500`, inline styles, and best practices.
  </Accordion>
...
| **Group variants**       | ❌ Parsed, no runtime effect                              | ✨ `group-active:*` / `group-focus:*` via shadow tree |
...
<Columns>
  ...
  <Card title="Zero Re-renders" icon="sparkles">
    ShadowTree updates inject styles without waking React. 55+ props across 20 components update on the native thread with no re-render cycle.
  </Card>
  ...
  <Card title="Group Variants" icon="object-group">
    `group-active:*` and `group-focus:*` propagate parent interaction state through the C++ shadow tree. Zero re-renders, zero context providers.
  </Card>
...
<Update label="June 11, 2026" description="1.4.0">
  ## Uniwind Pro 1.4.0
...
<Update label="June 3, 2026" description="1.3.0">
  ## Uniwind Pro 1.3.0
...
<Update label="June 1, 2026" description="1.2.1">
  ## Uniwind Pro 1.2.1
...
<Update label="May 21, 2026" description="1.2.0">
  ## Uniwind Pro 1.2.0
...
<Update label="May 14, 2026" description="1.1.3">
  ## Uniwind Pro 1.1.3
...
<Update label="May 11, 2026" description="1.1.2">
  ## Uniwind Pro 1.1.2
...
<Update label="May 5, 2026" description="1.1.1">
  ## Uniwind Pro 1.1.1
...
<Update label="April 23, 2026" description="1.1.0">
  ## Uniwind Pro 1.1.0
...
<Update label="April 7, 2026" description="1.0.1">
  ## Uniwind Pro 1.0.1
...
<AccordionGroup>
  <Accordion title="Show release candidate history">
    <Update label="March 26, 2026" description="1.0.0-rc.7">
    ...
    <Update label="March 19, 2026" description="1.0.0-rc.6">
...
Starting with Uniwind Pro **1.4.0**, every payload downloaded from the CDN is verified automatically. The CLI fetches the published SHA-256 checksum, validates the tarball before extracting it, and re-validates the local cache on every restore. If verification fails, nothing is installed.

You can also verify a download yourself. Each version's checksum is publicly available (no license required):

```bash theme={null}
curl https://cdn.uniwind.dev/uniwind-pro/checksums/<version>
# <sha256-hex>  uniwind-pro-<version>.tgz
```

The output is `sha256sum -c` compatible, so you can pin it in your own tooling:

```bash theme={null}
curl -s https://cdn.uniwind.dev/uniwind-pro/checksums/<version> | sha256sum -c
```

Download responses also carry a [RFC 9530](https://www.rfc-editor.org/rfc/rfc9530) `Repr-Digest: sha-256=:...:` header, and the same hash is printed in each GitHub release.

## Troubleshooting

<AccordionGroup>
  <Accordion title="Integrity verification failed">
    The downloaded payload did not match the published checksum, or the checksum endpoint was unreachable. The install aborts without writing any files. Check that:

    * Your network path to `cdn.uniwind.dev` is not intercepted by a proxy that rewrites responses
    * You can reach `https://cdn.uniwind.dev/uniwind-pro/checksums/<version>` directly

    Retry the install. If the failure persists, contact [support@uniwind.dev](mailto:support@uniwind.dev) immediately - do not bypass the check.
  </Accordion>
...
# Default Styles
Source: https://docs.uniwind.dev/pro/default-styles
...
# Group Variants
Source: https://docs.uniwind.dev/pro/group-variants
  ...
  <Accordion icon="terminal" title="Installation">
    ...
    <CodeGroup>
      ```bash better-t-stack theme={null}
        npx create-better-t-stack@latest --template uniwind
      ```
...
# Style Specificity
Source: https://docs.uniwind.dev/style-specificity
...
Set up themes in your React Native app with Uniwind. CSS-first theming with @theme, dark mode, custom colors, and dynamic theme switching — no ThemeProvider needed.
...
Source: https://docs.uniwind.dev/theming/custom-utilities
...
# Vite
Source: https://docs.uniwind.dev/vite
